import { similarity } from "@/utils/textNormalizer";
...
export const PROVINSI_LIST = PROVINCES_DATA.map((p) => p.name);

export const ALL_REGENCIES_DATA: Regency[] = [
  {
    id: "1101",
    province_id: "11",
...
async function cachedFetch<T>(url: string): Promise<T> {
  ...
  if (!response.ok) throw new Error(`Gagal memuat data wilayah: ${url}`);
...
export async function fetchDistricts(regencyId: string): Promise<District[]> {
  return cachedFetch<District[]>(`${BASE}/districts/${regencyId}.json`);
}

export async function fetchVillages(districtId: string): Promise<Village[]> {
  return cachedFetch<Village[]>(`${BASE}/villages/${districtId}.json`);
}

export function getProvinceIdByName(provinsiName: string): string | null {
  if (!provinsiName) return null;
  const clean = provinsiName
    .trim()
    .toUpperCase()
    .replace(/^PROVINS[I1]\s*/i, "");
  const exact = PROVINCES_DATA.find((p) => p.name === clean);
  if (exact) return exact.id;
  const found = PROVINCES_DATA.find((p) => p.name.includes(clean) || clean.includes(p.name));
  return found ? found.id : null;
}

export function getKabupatenKota(provinsi: string): string[] {
  if (!provinsi) return [];
  const provId = getProvinceIdByName(provinsi);
  if (!provId) return [];
  return ALL_REGENCIES_DATA.filter((r) => r.province_id === provId).map((r) => r.name);
}

export async function getKabupatenKotaAsync(provinsi: string): Promise<string[]> {
  return getKabupatenKota(provinsi);
}

export function getKecamatan(provinsi: string, kabupatenKota: string): string[] {
  if (!provinsi || !kabupatenKota) return [];
  const provId = getProvinceIdByName(provinsi);
  if (!provId) return [];

  const cleanKab = kabupatenKota
    .toUpperCase()
    .replace(/^KABUPATEN\s+UPATEN/i, "KABUPATEN")
    .trim();

  const coreKab = cleanKab.replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();

  const regency = ALL_REGENCIES_DATA.find((r) => {
    if (r.province_id !== provId) return false;
    const rCore = r.name
      .toUpperCase()
      .replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "")
      .trim();
    return (
      r.name.toUpperCase() === cleanKab ||
      rCore === coreKab ||
      rCore.includes(coreKab) ||
      coreKab.includes(rCore)
    );
  });

  if (!regency) return [];
  const list = (districtsData as unknown as Record<string, { id: string; name: string }[]>)[
    regency.id
  ];
  return list ? list.map((d) => d.name) : [];
}

export async function getKecamatanAsync(
  provinsi: string,
  kabupatenKota: string,
): Promise<string[]> {
  const sync = getKecamatan(provinsi, kabupatenKota);
...
export async function getDesaAsync(
  provinsi?: string,
  kabupatenKota?: string,
  kecamatan?: string,
): Promise<string[]> {
  ...
  let targetDistrictId: string | null = null;
  const cleanKec = kecamatan
    .trim()
    .toUpperCase()
    .replace(/^(KECAMATAN|KEC\.?)\s*/i, "");

  // 1. Coba cari dalam kabupaten & provinsi yang diberikan
  if (provinsi && kabupatenKota) {
    const provId = getProvinceIdByName(provinsi);
    if (provId) {
      const regency = ALL_REGENCIES_DATA.find((r) => {
        if (r.province_id !== provId) return false;
        const matched = matchKabupaten(kabupatenKota, [r.name]);
        return Boolean(matched);
      });
      if (regency) {
        const districtList =
          (districtsData as unknown as Record<string, { id: string; name: string }[]>)[
            regency.id
          ] || [];
        const matchedName = matchKecamatan(
          cleanKec,
          districtList.map((d) => d.name),
        );
        const d = districtList.find(
          (item) =>
            item.name === matchedName ||
            item.name.toUpperCase() === cleanKec ||
            similarity(item.name.toUpperCase(), cleanKec) >= 0.5,
        );
  ...
  // 2. Fallback: cari kecamatan di seluruh database lokal districts.json
  if (!targetDistrictId) {
    const allDistrictsMap = districtsData as unknown as Record<
      string,
      { id: string; name: string }[]
    >;
    let bestDist: { id: string; name: string } | null = null;
    let bestScore = 0;

    for (const [, list] of Object.entries(allDistrictsMap)) {
      for (const d of list) {
        ...
        const score = similarity(cleanKec, dUpper);
        if (score > bestScore) {
          bestScore = score;
          bestDist = d;
        }
  ...
  try {
    const villages = await fetchVillages(targetDistrictId);
...
export function matchProvinsi(candidate: string): string {
  if (!candidate) return "";
  // Strip common OCR prefixes and noise
  ...
  // 1. Exact match
  const exact = PROVINSI_LIST.find((p) => p === clean);
  if (exact) return exact;

  // 2. Substring match (both directions) — must return from official list
  const substring = PROVINSI_LIST.find((p) => p.includes(clean) || clean.includes(p));
  if (substring) return substring;

  // 3. Token-based: every significant word in clean must appear in province name
  const tokens = clean.split(" ").filter((t) => t.length >= 3);
  if (tokens.length > 0) {
    const tokenMatch = PROVINSI_LIST.find((p) =>
      tokens.every((t) => p.includes(t)),
    );
    if (tokenMatch) return tokenMatch;
  }

  // 4. Fuzzy similarity — always return from PROVINSI_LIST
  let bestProvince = "";
  let bestScore = 0;
  for (const p of PROVINSI_LIST) {
    const score = similarity(clean, p);
    if (score > bestScore) {
      bestScore = score;
      bestProvince = p;
    }
  }
  if (bestScore >= 0.5) return bestProvince;

  return "";
}

export function matchKabupaten(candidate: string, regencies: string[]): string {
  if (!candidate || !regencies.length) return "";
  let clean = candidate
    .toUpperCase()
    .replace(/KABUPATEN\s+UPATEN/gi, "KABUPATEN")
    .replace(/KABUPATEN\s+KABUPATEN/gi, "KABUPATEN")
    .replace(/\bKOEAKA\b/gi, "KOLAKA")
    .replace(/\bKOAKA\b/gi, "KOLAKA")
  ...
  // 1. Exact match (full name including prefix)
  const exact = regencies.find((r) => r.toUpperCase() === clean);
  if (exact) return exact;

  // Strip prefix for core-name matching
  const coreCandidate = clean.replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();

  // 2. Core exact match
  const coreExact = regencies.find((r) => {
    const coreR = r.toUpperCase().replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();
    return coreR === coreCandidate;
  });
  if (coreExact) return coreExact;

  // 3. Substring match on core names
  const coreSub = regencies.find((r) => {
    const coreR = r.toUpperCase().replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();
    return coreR.includes(coreCandidate) || coreCandidate.includes(coreR);
  });
  if (coreSub) return coreSub;

  // 4. Token-based: each significant token must appear in regency core name
  const tokens = coreCandidate.split(" ").filter((t) => t.length >= 3);
  if (tokens.length > 0) {
    const tokenMatch = regencies.find((r) => {
      const coreR = r.toUpperCase().replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();
      return tokens.every((t) => coreR.includes(t));
    });
    if (tokenMatch) return tokenMatch;
  }
  ...
  for (const r of regencies) {
    const coreR = r.toUpperCase().replace(/^(KABUPATEN|KOTA|KAB\.?)\s+/i, "").trim();
    const score = similarity(coreCandidate, coreR);
    if (score > bestScore) {
      bestScore = score;
      bestReg = r;
    }
  }
  if (bestScore >= 0.45) return bestReg;

  return "";
}

export function matchKecamatan(candidate: string, districts: string[]): string {
  if (!candidate || !districts.length) return "";
  const clean = candidate
    .toUpperCase()
    .replace(/^(KECAMATAN|KEC\.?)\s*/i, "")
    .replace(/[^A-Z0-9 ]/g, " ")
  ...
  // 1. Exact match
  const exact = districts.find((d) => d.toUpperCase() === clean);
  if (exact) return exact;

  // 2. Substring or token match (both directions)
  const tokenMatch = districts.find((d) => {
    const dUpper = d.toUpperCase();
    return dUpper === clean || dUpper.includes(clean) || clean.includes(dUpper);
  });
  if (tokenMatch) return tokenMatch;

  // 3. Token-based: each word must appear in district name
  const tokens = clean.split(" ").filter((t) => t.length >= 3);
  if (tokens.length > 0) {
    const tokBased = districts.find((d) => {
      const dUpper = d.toUpperCase();
      return tokens.every((t) => dUpper.includes(t));
    });
    if (tokBased) return tokBased;
  }

  // 4. Fuzzy similarity (Levenshtein >= 0.45)
  let best = "";
  let highest = 0;
  for (const d of districts) {
    const score = similarity(clean, d.toUpperCase());
    if (score > highest) {
      highest = score;
      best = d;
    }
  ...
  return highest >= 0.45 ? best : "";
}

export function matchDesa(candidate: string, villages: string[]): string {
  if (!candidate || !villages.length) return "";
  const clean = candidate
    .toUpperCase()
    .replace(/^(KELURAHAN|DESA|KEL\.?|KEL\/DESA|KEI\/DESA|KEI\.?)\s*/i, "")
    .replace(/[^A-Z0-9 ]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  // 1. Exact match
  const exact = villages.find((v) => v.toUpperCase() === clean);
  if (exact) return exact;

  // 2. Substring match (both directions)
  const tokenMatch = villages.find((v) => {
    const vUpper = v.toUpperCase();
    return vUpper === clean || vUpper.includes(clean) || clean.includes(vUpper);
  });
  if (tokenMatch) return tokenMatch;
  ...
  const tokens = clean.split(" ").filter((t) => t.length >= 3);
  if (tokens.length > 0) {
    const tokBased = villages.find((v) => {
      const vUpper = v.toUpperCase();
      return tokens.every((t) => vUpper.includes(t));
    });
    if (tokBased) return tokBased;
  }
  ...
  for (const v of villages) {
    const score = similarity(clean, v.toUpperCase());
    if (score > highest) {
      highest = score;
      best = v;
    }
...
export async function searchByVillageAndDistrict(
  villageName?: string,
  districtName?: string,
  knownKabupaten?: string,
  knownProvinsi?: string,
): Promise<WilayahMatch | null> {
  if (!villageName && !districtName) return null;
  ...
  let searchProvinces = PROVINCES_DATA;
  if (knownProvinsi) {
    const matchedP = matchProvinsi(knownProvinsi);
    const pObj = PROVINCES_DATA.find((p) => p.name === matchedP);
    if (pObj) searchProvinces = [pObj];
  }

  // 1. Jika nama kecamatan ada, cari dulu kecamatan lewat local districts.json (sangat cepat)
  if (districtName) {
    const cleanKec = districtName.trim().toUpperCase().replace(/^(KECAMATAN|KEC\.?)\s*/i, "");

    for (const province of searchProvinces) {
      let regencies = ALL_REGENCIES_DATA.filter((r) => r.province_id === province.id);
      if (knownKabupaten) {
        const matchedKab = matchKabupaten(
          knownKabupaten,
          regencies.map((r) => r.name),
        );
        const rObj = regencies.find((r) => r.name === matchedKab);
        if (rObj) regencies = [rObj];
      }

      for (const regency of regencies) {
        const dList = allDistrictsMap[regency.id] || [];
        const matchedKecName = matchKecamatan(
          cleanKec,
          dList.map((d) => d.name),
        );
        const matchedD = dList.find(
          (d) =>
            d.name === matchedKecName ||
            d.name.toUpperCase() === cleanKec ||
            similarity(d.name.toUpperCase(), cleanKec) >= 0.5,
        );

        if (matchedD) {
          if (villageName) {
            try {
              const villages = await fetchVillages(matchedD.id);
              const matchedV = matchDesa(
                villageName,
                villages.map((v) => v.name),
              );
              const vObj = villages.find((v) => v.name === matchedV);
              return {
                provinsi: province.name,
                kabupatenKota: regency.name,
                kecamatan: matchedD.name,
                kelurahanDesa: vObj?.name ?? matchedV ?? villageName.toUpperCase().trim(),
              ...
              // Jika fetch village gagal, tetap kembalikan data kecamatan yang cocok
            }
          }
          ...
          return {
            ...
            kabupatenKota: regency.name,
            kecamatan: matchedD.name,
            kelurahanDesa: villageName ? villageName.toUpperCase().trim() : "",
            provinceId: province.id,
            regencyId: regency.id,
            districtId: matchedD.id,
            villageId: "",
  ...
  // 2. Jika kecamatan tidak ada / tidak cocok, tetapi desa ada: cari desa di seluruh kecamatan kandidat
  if (villageName) {
    const cleanDesa = villageName
      .trim()
      .toUpperCase()
      .replace(/^(KELURAHAN|DESA|KEL\.?|KEL\/DESA|KEI\/DESA|KEI\.?)\s*/i, "");
    ...
    for (const province of searchProvinces) {
      ...
      for (const regency of regencies) {
        ...
        for (const dist of dList) {
          try {
            const villages = await fetchVillages(dist.id);
            const matchedV = matchDesa(
              cleanDesa,
              villages.map((v) => v.name),
            );
            if (matchedV) {
              ...
              return {
                ...
                kabupatenKota: regency.name,
                kecamatan: dist.name,
                kelurahanDesa: vObj?.name ?? matchedV,
                provinceId: province.id,
                regencyId: regency.id,
                districtId: dist.id,
                villageId: vObj?.id ?? "",
...
export interface ResolvedWilayahHierarchy {
  provinsi: string;
  kabupatenKota: string;
  kecamatan: string;
  kelurahanDesa: string;
...
/**
 * Menyelesaikan seluruh hierarki wilayah dari 4 level (Provinsi, Kabupaten, Kecamatan, Desa)
 * dengan pencocokan kemiripan tinggi, reverse-fill jika ada level yang kosong,
 * dan memuat daftar opsi desa/kelurahan resmi.
 */
export async function resolveCompleteWilayahHierarchy(input: {
  provinsi?: string;
  kabupatenKota?: string;
  kecamatan?: string;
  kelurahanDesa?: string;
  nik?: string;
}): Promise<ResolvedWilayahHierarchy> {
  const allDistrictsMap = districtsData as unknown as Record<
    string,
    { id: string; name: string }[]
  >;

  let prov = matchProvinsi(input.provinsi || "");
  let kab = "";
  let kec = "";
  let desa = input.kelurahanDesa?.toUpperCase().trim() || "";
  let desaOptions: string[] = [];

  // 1. NIK prefix hints
  const nik = (input.nik || "").replace(/\D/g, "");
  const nikProvId = nik.length >= 2 ? nik.slice(0, 2) : "";
  const nikRegId = nik.length >= 4 ? nik.slice(0, 4) : "";

  if (!prov && nikProvId) {
    const p = PROVINCES_DATA.find((item) => item.id === nikProvId);
    if (p) prov = p.name;
  }

  // 2. Kabupaten resolution
  if (prov) {
    const kabList = getKabupatenKota(prov);
    kab = matchKabupaten(input.kabupatenKota || "", kabList);
  }
  if (!kab && nikRegId) {
    const r = ALL_REGENCIES_DATA.find((item) => item.id === nikRegId);
    if (r) {
      kab = r.name;
      if (!prov) {
        const p = PROVINCES_DATA.find((item) => item.id === r.province_id);
        if (p) prov = p.name;
      }
    }
  }
  if (!kab && input.kabupatenKota) {
    kab = matchKabupaten(
      input.kabupatenKota,
      ALL_REGENCIES_DATA.map((r) => r.name),
    );
    if (kab) {
      const r = ALL_REGENCIES_DATA.find((item) => item.name === kab);
      if (r && !prov) {
        const p = PROVINCES_DATA.find((item) => item.id === r.province_id);
        if (p) prov = p.name;
      }
  ...
  // 3. Kecamatan resolution
  let resolvedDistrictId: string | null = null;

  if (prov && kab) {
    const kecList = getKecamatan(prov, kab);
    kec = matchKecamatan(input.kecamatan || "", kecList);
    if (kec) {
      const reg = ALL_REGENCIES_DATA.find((r) => r.name === kab);
      if (reg) {
        const dList = allDistrictsMap[reg.id] || [];
        const d = dList.find((item) => item.name === kec);
        if (d) resolvedDistrictId = d.id;
      }
    }
  }

  // 4. Reverse resolution jika ada level yang masih kosong
  if (!kec || !kab || !prov) {
    const reverseMatch = await searchByVillageAndDistrict(
      input.kelurahanDesa || undefined,
      input.kecamatan || undefined,
      kab || input.kabupatenKota || undefined,
      prov || input.provinsi || undefined,
    );

    if (reverseMatch) {
      if (!prov && reverseMatch.provinsi) prov = reverseMatch.provinsi;
      if (!kab && reverseMatch.kabupatenKota) kab = reverseMatch.kabupatenKota;
      if (!kec && reverseMatch.kecamatan) kec = reverseMatch.kecamatan;
      if (reverseMatch.districtId) resolvedDistrictId = reverseMatch.districtId;
      if (reverseMatch.kelurahanDesa) desa = reverseMatch.kelurahanDesa;
    }
  }

  // 5. Muat daftar desa untuk kecamatan yang telah teridentifikasi & cocokkan desa
  if (prov && kab && kec) {
    try {
      desaOptions = await getDesaAsync(prov, kab, kec);
      if (desaOptions.length > 0 && desa) {
        const matchedD = matchDesa(desa, desaOptions);
        if (matchedD) {
          desa = matchedD;
        }
      }
  ...
  return {
    provinsi: prov || input.provinsi || "",
    kabupatenKota: kab || input.kabupatenKota || "",
    kecamatan: kec || input.kecamatan || "",
    kelurahanDesa: desa || input.kelurahanDesa || "",
    desaOptions,
  };
}
